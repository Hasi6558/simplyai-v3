  ...
  try {
    ...
    if (!user_id || !questionnaire_id || !responses) {
      return res.status(400).json({
        success: false,
        message: 'Missing required fields: user_id, questionnaire_id, responses'
      });
    }
    ...
    const createdAtMysql = created_at ? new Date(created_at).toISOString().slice(0, 19).replace('T', ' ') : mysqlDatetime;
