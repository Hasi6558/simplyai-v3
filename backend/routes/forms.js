...
router.post("/", async (req, res) => {
  const { id, title, description, surveyJSON, logo, status, createdBy } =
    req.body;
  ...
  try {
    ...
    if (id) {
      ...
      if (result.affectedRows === 0) {
        return res
          .status(404)
          .json({ success: false, message: "Form not found for update", id });
      }
      res.json({
        success: true,
      ...
      // Create new form
      console.log(
        "Creating new form with SQL:",
        `INSERT INTO questionnaire_config (title, description, questions, logo, status, created_by) VALUES (?, ?, ?, ?, ?, ?)`
      );
