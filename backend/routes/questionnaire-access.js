...
router.get('/user-questionnaires-with-access', async (req, res) => {
  try {
    ...
    const planOptions = typeof userPlan.plan_options === 'string' 
    ...
    if (questionnaireIds.length > 0) {
      const [completionRows] = await pool.query(`
        ...
        WHERE user_id = ? AND subscription_id = ? AND questionnaire_id IN (${questionnaireIds.map(() => '?').join(',')})
    ...
    const questionnairesWithAccess = questionnaireRows.map(questionnaire => {
      ...
      return {
        ...
        completionCount: completions.filter(c => c.status === 'completed').length,
        lastCompletedAt: completions.length > 0 ? completions[0].completed_at : null
      };
    });
 ...
 */
function calculateQuestionnaireAccess(questionnaire, completions, planOptions, userPlan, allQuestionnaires = null, allCompletionHistory = null) {
  console.log(`\n=== ACCESS CALCULATION for Questionnaire ${questionnaire.id} ===`);
  console.log('Plan type:', userPlan.plan_type);
  console.log('Plan options:', planOptions);
  console.log('Questionnaire sequence order:', questionnaire.sequence_order);
  ...
  const lastCompletedDate = completedAttempts.length > 0 ? new Date(completedAttempts[0].completed_at) : null;
  const now = new Date();
  ...
  if (planOptions.verificationAfter) {
    if (completionCount === 0) {
      ...
      if (lastCompletedDate) {
        ...
        nextAvailable.setDate(nextAvailable.getDate() + (planOptions.verificationPeriod || 90));
  ...
  if (planOptions.periodicQuestionnaires) {
    ...
    console.log(`🔄 Periodic Questionnaires Logic:`, {
    ...
    if (completionCount < maxReps && lastCompletedDate) {
      ...
      console.log(`⏱️ Periodic waiting period check:`, {
        lastCompletedDate: lastCompletedDate.toISOString(),
        nextAvailable: nextAvailable.toISOString(),
        currentDate: now.toISOString(),
        canAccess: now >= nextAvailable
      });
      ...
      if (now >= nextAvailable) {
        return {
          status: 'available',
          canAccess: true,
          reason: `Compilazione ${completionCount + 1} di ${maxReps} disponibile`
        };
      } else {
        const daysRemaining = Math.ceil((nextAvailable.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
        return {
  ...
  if (planOptions.progressQuestionnaires) {
    ...
    if (questionnaire.sequence_order === 1) {
      console.log('This is the first questionnaire (sequence_order = 1), allowing access');
    ...
    console.log('Looking for previous questionnaire with sequence_order:', previousSequenceOrder);
    
    const previousQuestionnaire = allQuestionnaires.find(q => q.sequence_order === previousSequenceOrder);
    ...
    const previousCompletions = allCompletionHistory[previousQuestionnaire.id] || [];
    const previousCompleted = previousCompletions.filter(c => c.status === 'completed');
    
    console.log('Previous questionnaire completions:', previousCompleted.length);
    ...
    if (now < nextAvailable) {
      const daysToWait = Math.ceil((nextAvailable - now) / (1000 * 60 * 60 * 24));
  ...
  if (planOptions.singleQuestionnaire || (!planOptions.multipleQuestionnaires && !planOptions.verificationAfter && !planOptions.periodicQuestionnaires && !planOptions.progressQuestionnaires)) {
    if (completionCount === 0) {
      return {
        status: 'available',
        canAccess: true,
