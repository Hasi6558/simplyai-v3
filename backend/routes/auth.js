...
async function getDefaultFreePlanId() {
  ...
  try {
    ...
    console.error('Error getting default free plan from subscription_plans table:', error);
    throw new Error('Unable to find subscription_plans table or get default plan');
  }
}
...
router.post('/register', async (req, res) => {
  try {
      ...
      if (!uuidRegex.test(subscription_plan)) {
        ...
        try {
          ...
          if (planResult.length > 0) {
            ...
            console.log('✅ Found registration plan by name:', subscription_plan, '-> ID:', finalPlanId);
          } else {
        ...
        try {
          ...
          if (planResult.length > 0) {
            ...
            const plan = planResult[0];
            isFreeRegistration = plan.is_free || plan.price === 0;
            console.log('✅ Registration plan ID format detected:', subscription_plan);
          } else {
            return res.status(400).json({
              success: false,
              message: 'Invalid subscription plan selected'
    ...
    if (isFreeRegistration) {
      ...
      console.log('🆓 Creating free plan user:', { userId, email, subscription_plan });
      ...
      try {
        ...
        const subscriptionPlanId = finalPlanId || await getDefaultFreePlanId();
        console.log('📋 Creating subscription record for plan ID:', subscriptionPlanId);
        await connection.execute(
          'INSERT INTO user_subscriptions (id, user_id, plan_id, status, started_at, created_at, updated_at) VALUES (?, ?, ?, ?, NOW(), NOW(), NOW())',
        ...
        try {
          ...
          console.error('Failed to send welcome email to free plan user:', emailError);
        ...
        console.error('❌ Transaction failed during free user registration:', error);
      ...
      try {
        ...
        console.log('💳 Paid plan user stored temporarily for payment:', userId);

        // Return user ID for payment processing (no auth token yet)
        res.status(201).json({
          success: true,
          message: 'Registration data validated. Please complete payment to finalize account.',
          data: {
            tempUserId: userId,
            email,
            firstName,
...
router.get('/google/callback', 
  ...
  async (req, res) => {
    try {
      ...
      // Existing user - update last_activity and generate JWT token and redirect to dashboard
      await pool.execute(
        'UPDATE profiles SET last_activity = NOW() WHERE id = ?',
        [req.user.id]
      );
    ...
    if (subscription_plan) {
      ...
      if (!uuidRegex.test(subscription_plan)) {
        ...
        try {
          ...
          if (planResult.length > 0) {
            finalPlanId = planResult[0].id;
            console.log('✅ Found plan by name:', subscription_plan, '-> ID:', finalPlanId);
          } else {
            console.warn('⚠️ Plan not found, using default free plan:', subscription_plan);
            finalPlanId = await getDefaultFreePlanId();
          }
        } catch (error) {
          console.error('❌ Error finding plan by name, using default free plan:', error);
          finalPlanId = await getDefaultFreePlanId();
        }
      } else {
    ...
    try {
      ...
      await connection.execute(
        `INSERT INTO profiles (id, email, first_name, last_name, full_name, google_id, created_at, updated_at) 
         VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())`,
        [userId, email, firstName, lastName, `${firstName} ${lastName}`, googleId]
      );
      ...
      try {
        const planName = subscription_plan === 'free' ? 'Piano Gratuito' : subscription_plan;
        const isFreeRegistration = !subscription_plan || subscription_plan === 'free' || subscription_plan === 'Piano Gratuito';
        ...
        if (isFreeRegistration) {
          ...
          console.log('✅ Welcome email sent to Google user (free plan):', email);
        } else {
          // If it's a paid plan, include payment info - get actual plan price
          const transactionId = `OAUTH_${Date.now()}_${uuidv4().substring(0, 8)}`;
          ...
          try {
            ...
            console.error('Error fetching plan price for OAuth registration:', error);
          }
          ...
          console.log('✅ Welcome email sent to Google user (paid plan):', email, 'Price:', actualPlanPrice);
        }
      } catch (emailError) {
    ...
    if (subscription_plan) {
      ...
      if (!uuidRegex.test(subscription_plan)) {
        ...
        try {
          ...
          if (planResult.length > 0) {
            finalPlanId = planResult[0].id;
            console.log('✅ Found Facebook plan by name:', subscription_plan, '-> ID:', finalPlanId);
          } else {
            console.warn('⚠️ Facebook plan not found, using default free plan:', subscription_plan);
            finalPlanId = await getDefaultFreePlanId();
          }
        } catch (error) {
          console.error('❌ Error finding Facebook plan by name, using default free plan:', error);
          finalPlanId = await getDefaultFreePlanId();
        }
      } else {
    ...
    try {
      ...
      await connection.execute(
        `INSERT INTO profiles (id, email, first_name, last_name, full_name, facebook_id, created_at, updated_at) 
         VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())`,
        [userId, email, firstName, lastName, `${firstName} ${lastName}`, facebookId]
      );
      ...
      try {
        ...
        if (isFreeRegistration) {
          ...
          console.log('✅ Welcome email sent to Facebook user (free plan):', email);
        } else {
          // If it's a paid plan, include payment info - get actual plan price
          const transactionId = `OAUTH_${Date.now()}_${uuidv4().substring(0, 8)}`;
          ...
          try {
            ...
            console.error('Error fetching plan price for Facebook OAuth registration:', error);
          }
          ...
          console.log('✅ Welcome email sent to Facebook user (paid plan):', email, 'Price:', actualPlanPrice);
        }
      } catch (emailError) {
...
router.post('/register/complete-with-plan', async (req, res) => {
  try {
    ...
    console.log('Completing registration with plan:', { email: tempUserData?.email, planId });

    // Validate required fields
    if (!tempUserData || !tempUserData.email || !tempUserData.password || !tempUserData.firstName || !tempUserData.lastName) {
      return res.status(400).json({
        success: false,
    ...
    if (!uuidRegex.test(planId)) {
      ...
      try {
        ...
        if (planResult.length > 0) {
          finalPlanId = planResult[0].id;
          console.log('✅ Found complete-with-plan by name:', planId, '-> ID:', finalPlanId);
        }
      } catch (error) {
    ...
    try {
      ...
      console.log('✅ User registered successfully with plan:', userId, plan.name);
      ...
      try {
        if (isFreeRegistration) {
          ...
          console.log('✅ Free plan registration confirmation email sent:', email, plan.name);
        } else {
          ...
          console.log('✅ Paid plan registration confirmation email sent with payment info:', email, plan.name);
        }
      } catch (emailError) {
...
router.delete('/cleanup-expired-registrations', async (req, res) => {
  try {
    ...
    console.log(`🧹 Cleaned up ${result.affectedRows} expired temporary registrations`);
