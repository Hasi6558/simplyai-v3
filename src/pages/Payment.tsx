...
import { Check, Clock, CreditCard, Lock, Loader2, Calendar, User, Mail } from 'lucide-react';
import { supabase } from '@/integrations/supabase/client';
import { registerUserWithGoogle, registerUserWithFacebook, processPayment, processOAuthPayment, completeRegistrationWithPlan, fetchPlan } from '../services/ApiService';
import { useAuth } from '@/contexts/AuthContext';
import { Elements } from '@stripe/react-stripe-js';
import { loadStripe } from '@stripe/stripe-js';
import StripeCheckoutForm from '@/components/StripeCheckoutForm';
import { fetchPaymentSettings, formatCurrency, getCurrencySymbol, type PaymentSettings } from '@/services/paymentService';
...
const Payment = () => {
  ...
  useEffect(() => {
    ...
    if (googleSignup === 'true' && googleDataParam) {
      try {
        const parsedGoogleData = JSON.parse(decodeURIComponent(googleDataParam));
    ...
    if (facebookSignup === 'true' && facebookDataParam) {
      try {
        ...
        const parsedFacebookData = JSON.parse(decodeURIComponent(facebookDataParam));
        console.log('✅ Facebook data parsed successfully:', parsedFacebookData);
        ...
        console.error('Decoded facebookDataParam:', decodeURIComponent(facebookDataParam));
      }
    } else {
    ...
    if (isTempUser && tempUserDataStr) {
      try {
        ...
        console.log('📋 New temp user data loaded for payment:', parsedTempData.email);
      } catch (error) {
    ...
    if (googleDataParam) {
      ...
      console.log('Decoded Google Data Param:', decodeURIComponent(googleDataParam));
    }
    if (googleData) {
    ...
    if (facebookDataParam) {
      ...
      console.log('Decoded Facebook Data Param:', decodeURIComponent(facebookDataParam));
    }
    if (facebookData) {
      console.log('Parsed Facebook Data:', JSON.stringify(facebookData, null, 2));
    } else {
    ...
    console.log('Local Storage Auth Token:', localStorage.getItem('authToken') ? 'Present' : 'Not Found');
    console.log('Local Storage User:', localStorage.getItem('user') ? 'Present' : 'Not Found');
      ...
      if (isPlanFree && planName) {
        ...
        if (isGoogleSignup && googleData) {
          try {
            ...
            console.log('🚀 Starting Google registration for free plan (from separate effect):', {
              planId: selectedPlanId,
              planName: planName,
              googleData: googleData
            });
            
            const registrationResponse = await registerUserWithGoogle(googleData, planName);
            
            if (registrationResponse.success) {
              // Store the token and user data using the correct keys that AuthContext expects
              localStorage.setItem('auth_token', registrationResponse.data.token);
              localStorage.setItem('user', JSON.stringify(registrationResponse.data.user));
              
              console.log('✅ Google registration successful (from separate effect):', {
                userId: registrationResponse.data.user.id,
                email: registrationResponse.data.user.email,
                token: registrationResponse.data.token ? 'Present' : 'Missing'
              });
              ...
              console.error('❌ Google registration failed (from separate effect):', registrationResponse);
              throw new Error(registrationResponse.message || 'Registration failed');
            }
          } catch (error) {
            console.error('Error completing Google registration (from separate effect):', error);
            toast({
              variant: 'destructive',
        ...
        if (isFacebookSignup && facebookData) {
          try {
            ...
            console.log('🚀 Starting Facebook registration for free plan (from separate effect):', {
              planId: selectedPlanId,
              planName: planName,
              facebookData: facebookData
            });
            
            const registrationResponse = await registerUserWithFacebook(facebookData, planName);
            ...
            if (registrationResponse.success) {
              ...
              console.log('✅ Facebook registration successful (from separate effect):', {
                userId: registrationResponse.data.user.id,
                email: registrationResponse.data.user.email,
                token: registrationResponse.data.token ? 'Present' : 'Missing'
              });
              ...
              console.error('❌ Facebook registration failed (from separate effect):', registrationResponse);
              throw new Error(registrationResponse.message || 'Registration failed');
            }
          } catch (error) {
            console.error('Error completing Facebook registration (from separate effect):', error);
            toast({
              variant: 'destructive',
              title: 'Registration Error',
              description: 'There was an error completing your registration. Please try again.',
  ...
  }, [isPlanFree, planName, isGoogleSignup, googleData, isFacebookSignup, facebookData, selectedPlanId, toast]);
  ...
  const handlePayment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isTempUser && tempUserData.email && tempUserData.password) {
        console.log('🔥 PRIORITY: Processing temp user registration with plan selection...');
        console.log('📝 Temp user data:', { email: tempUserData.email, firstName: tempUserData.firstName });
        
        const response = await completeRegistrationWithPlan(tempUserData, selectedPlanId);
        ...
        if (response.success) {
          ...
          console.log('✅ Temp user registration with plan completed successfully');
      ...
      if (isAuthenticated && user) {
        ...
        if (!planInfo.id) {
          ...
          const recentPlanId = localStorage.getItem('recent_plan_id') || localStorage.getItem('user_selected_plan_id');
          const recentPlanPrice = localStorage.getItem('recent_plan_price') || localStorage.getItem('user_selected_plan_price');
          const recentPlanName = localStorage.getItem('recent_plan_name') || localStorage.getItem('user_selected_plan_name');
        ...
        if (!user.id) {
          ...
          toast({
            ...
            description: 'ID utente mancante. Prova a effettuare nuovamente il login.',
          });
          return;
        }
        ...
        if (!userInfo.email) {
          ...
          toast({
            ...
            description: 'Email utente mancante. Prova a effettuare nuovamente il login.',
          });
          return;
        }
        ...
        if (!planInfo.id) {
          ...
          toast({
            variant: 'destructive',
            title: 'Errore piano',
            description: 'ID piano mancante. Riprova selezionando un piano dalla pagina principale.',
          });
        ...
        console.log('✅ All required fields validated, proceeding with payment...');

        // Process payment for authenticated user and send notification email
        const response = await processOAuthPayment(user.id, userInfo, paymentInfo, planInfo);
      ...
      if (authToken && userData) {
        ...
        console.log('🔐 Processing payment for legacy OAuth user:', currentUser);
        ...
        const response = await processOAuthPayment(currentUser.id, userInfo, paymentInfo, planInfo);
      ...
      // If this is a Facebook signup, complete the registration
        ...
        const paymentInfo = {
          ...
          cardNumber: formData.get('cardNumber') ? `****-****-****-${String(formData.get('cardNumber')).slice(-4)}` : undefined
        };
        ...
        if (!tempUserData.tempUserId && tempUserData.email && tempUserData.password) {
          ...
          const response = await processPayment(userInfo, paymentInfo, planInfo, tempUserData.tempUserId);
          ...
          if (response.success) {
            ...
            if (tempUserData.tempUserId) {
              ...
              console.log('🧹 Cleaned up temporary user data after successful payment');
            }
      ...
      toast({
        variant: 'destructive',
        title: (isGoogleSignup || isFacebookSignup) ? 'Registration Error' : 'Errore di pagamento',
        description: (isGoogleSignup || isFacebookSignup)
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                      <User size={20} />
                      {tempUserData.tempUserId ? 'Informazioni Account (da registrazione)' : 'Informazioni Account'}
                    </CardTitle>
                    <CardDescription>
                  ...
                  <CardContent className="space-y-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          <Input 
                            ...
                            className={tempUserData.tempUserId ? 'bg-gray-50' : ''}
                          />
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-sm text-gray-500">
                        Se non inserisci una password, ne genereremo una automaticamente e te la invieremo via email.
                      </p>
                    </div>
                  </CardContent>
                </Card>
                    ...
                    <TabsContent value="card">
                      {currentStripePromise ? (
                          ...
                          <StripeCheckoutForm
                            ...
                            userInfo={
                              isGoogleSignup && googleData
                                ? {
                                    firstName: googleData.firstName || googleData.given_name || '',
                                    lastName: googleData.lastName || googleData.family_name || '',
                                    email: googleData.email || '',
                                  }
                                : isFacebookSignup && facebookData
                                ? {
                                    firstName: facebookData.firstName || facebookData.given_name || '',
                                    lastName: facebookData.lastName || facebookData.family_name || '',
                            ...
                            onSuccess={() => {
                              toast({
                                title: 'Pagamento completato!',
                                description: 'Il pagamento con carta è andato a buon fine.',
                              });
                              setTimeout(() => {
                                navigate('/dashboard');
                              }, 2000);
                        ...
                        <div className="flex justify-center items-center py-8">
                          ...
                          <span className="ml-2">Caricamento configurazione pagamenti...</span>
                        </div>
                      )}
                    </TabsContent>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-bold">
                        {paymentSettings 
                          ? formatCurrency(selectedPlanPrice * 100, paymentSettings.currency)
